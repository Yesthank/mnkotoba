...
이 앱이 자체적으로 거는 제한은 「한 번에 2000자」 하나뿐입니다. 그 외에 걸리는 한도는 전부 구글 쪽입니다.

| 한도 | 기본값(2.5 Flash 무료) | 걸렸을 때 |
|---|---|---|
| 분당 요청 수 (RPM) | 10 | 앱이 알아서 기다렸다 다시 보냅니다 |
| 분당 토큰 수 (TPM) | 250,000 | 문장 단위로는 거의 안 걸립니다 |
| 하루 요청 수 (RPD) | 모델·시기에 따라 다름 | 태평양 시간 자정까지 기다려야 합니다 |

문장 하나 = 요청 하나입니다. 연달아 빠르게 넣으면 분당 한도부터 걸립니다.

- 앱은 최근 1분에 보낸 요청 수를 세고, 한도를 넘길 것 같으면 보내기 전에 기다립니다(대기열에 "분당 한도 대기"로 표시).
- 그래도 429가 오면 함수가 구글이 알려준 시간만큼 기다렸다 한 번 더 겁니다.
- 어느 한도에 걸렸는지는 오류 메시지에 그대로 나옵니다. "분당"이면 잠깐 기다리면 되고, "하루"면 그날은 끝입니다.

모델을 바꾸면 한도도 바뀝니다. `VITE_GEMINI_RPM` 에 그 모델의 RPM 을 넣어주면 앱이 그 수에 맞춰 간격을 잡습니다(기본 10). 한도가 계속 모자라면 [AI Studio](https://aistudio.google.com/apikey)에서 결제를 연결하는 쪽이 가장 확실합니다. 유료 1단계로 올라가면 분당 한도가 수백 단위로 뜁니다.
...
| `VITE_GEMINI_RPM` | 선택. 쓰는 모델의 분당 요청 한도. 기본 10 |
